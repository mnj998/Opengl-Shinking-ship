...
void display0() {
    ...
    glLineWidth(3);
    glColor3f(1.0, 0, 0);
    drawStrokeText("THE SINKING SHIP", WIDTH/3, HEIGHT/2, 0, FONT_SIZE_NORMAL);
...
{
    int key=0;
    printf("\t\t\t\tSRINIVAS INSTITUTE OF TECHNOLOGY, VALACHIL\n");
    printf("\t\t\t\tDEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING\n");
    printf("\t\t\t\tCOMPUTER GRAPHICS MINI PROJECT ON\n");
    printf("\t\t\t\t\tSINKING SHIP\n");
    printf("\n\n\n\t\t\t\t project by\n");
    printf("\n\t\t\t\t\t Manoj Krishna U");
    printf("\n\t\t\t\t\t Nagaraja N Poojary");
    printf("\n\n\n\n\n\t\t\t\t\tpress 1 to Enter the ship\n");
    printf("        \n\t\t\t\t\tpress 0 to abort and quit\n\n\t\t\t\t\t");
    scanf("%d",&key);
    if(key==0) return 0;
    glutInit(&argc, argv);
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB);
    glutInitWindowSize(1920.0,1080.0);
